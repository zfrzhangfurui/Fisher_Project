...
IF (@_cnt%1000 = 0 AND @_cnt <> 0) OR (@cnt = @row_num) THEN
    ...
    SELECT COUNT(*) INTO @temp_result FROM (SELECT W1_HORSE FROM scv.temp_horsnameT GROUP BY W1_HORSE HAVING COUNT(*) >= @least_run AND DATEDIFF(MAX(W1_DATE), MIN(W1_DATE) ) <= @daysBet) as TAB;
    
--     IF (@temp_result >= @num_result) THEN
--         LEAVE horse_1000;
--     END IF;
ELSE 
    SELECT COUNT(*) INTO @temp_sameDayRun FROM `scv`.`workrace` WHERE W_DATE = wdate AND W_HORSE = horsename;
